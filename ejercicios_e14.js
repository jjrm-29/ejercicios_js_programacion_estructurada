...
const nombreProductos = productos.map(prod => prod.nombre);
console.log(nombreProductos);

const todosProductosCategoriaMuebles = productos.filter(p => p.categoria === "Muebles");
console.log(todosProductosCategoriaMuebles);

const primerProductoPrecioMayorA1000 = productos.find(p => p.precio > 1000);
console.log(primerProductoPrecioMayorA1000);

const precioTotalInventario = productos.reduce((acc, prod) => acc + (prod.precio * prod.stock), 0);
console.log(precioTotalInventario);

const todosProductosConStockMayorACero = productos.every(p => p.stock > 0);
console.log(todosProductosConStockMayorACero);

const hayProductoCategoriaElectrodomesticos = productos.some(p => p.categoria === "Electrodomésticos");
console.log(hayProductoCategoriaElectrodomesticos);
